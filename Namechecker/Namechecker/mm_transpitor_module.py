...
def transcript(text: str) -> str:
    ...
    while i < len(text):
        ...
        # print('afterNext' + afterNext)
        thirdNext = text[i + 3] if i + 3 < len(text) else ""
        allButS = text[1:] if len(text) > 1 else ""
        print('allButS   ' + allButS)
        allButSC = text[2:] if len(text) > 2 else ""
        print('allButSC   ' + allButSC)
        allButSCC = text[3:] if len(text) > 3 else ""
        print('allButSCC   ' + allButSCC)
        ...
        # Rule 4.0: Singleton + AsatExtendedForm
            ...
            next in clusterForm and 
            afterNext in clusterForm and 
            allButSCC in vowelExtendedForm):
            if thirdNext == 'ှ':
                if ('ျ' in next or 'ြ' in next) and char in modifyForm:
                    output.append(
                        f"{vowelForm[thirdNext]}{modifyForm[char]}{clusterForm[afterNext]}{vowelExtendedForm[allButSCC]}")
                else:
                    output.append(
                        f"{vowelForm[thirdNext]}{singleton[char]}{clusterForm[next]}{clusterForm[afterNext]}{vowelExtendedForm[allButSCC]}")
                i += 3 + len(allButSCC)
            else:
                if ('ျ' in next or 'ြ' in next) and char in modifyForm:
        ...
        # Rule 3.0: Singleton + Cluster + Cluster + Vowel
            ...
            next in clusterForm and 
            afterNext in clusterForm):
            if thirdNext in vowelForm:
                if thirdNext == 'ှ':
                    if ('ျ' in next or 'ြ' in next) and char in modifyForm:
                        output.append(
                            f"{vowelForm[thirdNext]}{modifyForm[char]}{clusterForm[afterNext]}a")
                    else:
                        output.append(
                            f"{vowelForm[thirdNext]}{singleton[char]}{clusterForm[next]}{clusterForm[afterNext]}a")
                else:
                    if ('ျ' in next or 'ြ' in next) and char in modifyForm:
                        output.append(modifyForm[char] + 
                                    clusterForm[afterNext] + 
                ...
                if ('ျ' in next or 'ြ' in next) and char in modifyForm:
                    output.append(f"{modifyForm[char]}{clusterForm[afterNext]}a")
                else:
                    output.append(
                        f"{singleton[char]}{clusterForm[next]}{clusterForm[afterNext]}a")
                i += 3
        ...
        elif singleton.get(char) and vowelExtendedForm.get(allButS):
            if next == 'ှ':
                if char.contains('ရ'):
                    output.append("sh" + vowelExtendedForm[allButS])
                else:
                    output.append(vowelForm[next] + singleton[char] + vowelExtendedForm[allButS])
                i += 1 + len(allButS)
            else:
                output.append(singleton[char] + vowelExtendedForm[allButS])
                i += 1 + len(allButS)

        elif singleton.get(char):
            if vowelForm.get(next):
                if next == 'ှ':
                    if char.contains('ရ'):
                        output.append('sha')
                    else:
                        output.append(vowelForm[next] + singleton[char] + 'a')
                else:
                    output.append(singleton[char] + vowelForm[next])
                i += 2
            else:
                if char.contains('အ'):
                    output.append("ah")
                else:
                    output.append(singleton[char] + 'a')
                i += 1

        elif specialCharsForm.get(char):
